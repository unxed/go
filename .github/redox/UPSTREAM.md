...
| 4 | relibc | `posix_spawn` does not close, in the child, the parent's close-on-exec descriptors (a `cat` child keeps the write end of its own stdin pipe open and never sees EOF); a spawn that fails halfway (e.g. EBADF because a listed fd vanished) leaves a half-built child process holding copies of every fd | `x26_spawn_cloexec_c` | worked around in Go (explicit `addclose`, closes serialised with ForkLock); `x27_spawnpar_c` = concurrent variant |
| 5 | relibc | a new thread starts with an empty signal mask and applies the creator's mask a little later (pthread/mod.rs); a process-directed signal (SIGCHLD of an exited child) can hit it before TLS/`g` is set up | seen as "fatal: bad g in signal handler" in `x20_execpar` | worked around in Go (drop benign async signals on a g-less thread) |
| 6 | relibc | `sigentry` clobbers RCX of the interrupted code (async signals corrupt user state) | `x10_sigstorm` | fixed upstream: unxed/relibc `fix-sigentry-rcx-clobber` (8022058e) |
| 7 | QEMU TCG (not KVM) | CLOCK_MONOTONIC / CLOCK_REALTIME read on different vCPUs go backwards by up to ~4 ms | `x21_clock_c` | informational; gone with KVM |

Not a bug, but easy to trip over: a process created by `posix_spawn` starts with all-zero
FPU control state (MXCSR = 0 unmasks every SSE exception, and without CR4.OSXMMEXCPT the first
inexact result raises #UD). relibc's `crt0` sets MXCSR and the x87 control word (`src/crt0`), so
C programs are fine; an ELF entry point that bypasses crt0 (Go's) must do it itself.
